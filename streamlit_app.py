...
with st.sidebar:
    ...
    max_new_tokens = st.sidebar.slider('max_new_tokens', min_value=1000, max_value=20000, value=20000, step=14)
...
def generate_llama2_response(prompt_input):
    ...
    callminer logic i.e. "calling|called make payment|installment:2" which uses: AND logic = Use a blank space as a separator or the word AND. // OR logic = Use a vertical bar (|) or the word OR. // PHRASE logic = Use double quotes " " around a set of words to find the words in that exact order in a given timeframe. Don't use around single words. // CLOSE-TO logic = Use square brackets [ ] around a set of words to find those words in any order during a given timeframe. // NOT logic = Use a minus sign (-) immediately before a word/phrase, preceded by a space to rule out contacts with this term. Note: (-) operator cannot be used with proximity operators (BEFORE, AFTER, NEAR). // Distance Operators = Use a colon (:) followed by a numerical value (:2 or :2s)to specify a time frame in seconds for your search criteria for calls. //
    supervis* - Returns contacts containing all tenses of the word, including supervise, supervises, supervisor, and supervising. //
    speak supervisor - Returns contacts that have both terms present (they do not have to occur near each other or be in any particular order). //
    speak | supervisor - Returns contacts that have either term present. //
    “speak supervisor”:2 - Finds the words in this order within a 2-second window. Hits on phrase “speak to my supervisor”; does not hit on “you have a supervisor to speak to?”. // 
    “talk|speak supervisor”:3 - Finds phrase “talk supervisor” OR “speak supervisor” in that order, within a 3-second timeframe. Hits on phrase “speak to a supervisor” as well as “speak to your manager or your boss or supervisor”. The default timeframe is a calculation of (# of words * 0.3 seconds) – 0.1 second. Thus a 2-word phrase defaults to 0.5 seconds. //
    ...
    output = replicate.run('meta/meta-llama-3-8b-instruct', 
                           input={"prompt": f"{string_dialogue} {prompt_input} Assistant: ",
                                  "temperature":temperature, "top_p":top_p, "max_new_tokens":max_new_tokens, "repetition_penalty":1, context_length=20000})
    return output
